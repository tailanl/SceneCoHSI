...
class SOMASkeleton30(SkeletonBase):
    ...
    @ensure_batched(positions_30=3)  # [B, T, J30, 3]
    def expand_positions_to_77(self, positions_30: torch.Tensor):
        """Expand 30-joint positions to 77-joint positions using FK.

        Args:
            positions_30: Position tensor with shape [B, T, 30, 3]

        Returns:
            Expanded position tensor with shape [B, T, 77, 3]
        """
        device = positions_30.device
        nB, nT, _, _ = positions_30.shape
        
        # Use the 77-joint skeleton's neutral joints as base
        neutral_joints_77 = self.somaskel77.neutral_joints[None, None].to(device)  # [1, 1, 77, 3]
        
        # Get the 30-joint slice indices
        skel_slice = self.get_skel_slice(self.somaskel77)
        
        # Create expanded positions initialized with 77-joint neutral pose
        positions_77 = neutral_joints_77.repeat(nB, nT, 1, 1)
        
        # Copy the 30-joint positions to their corresponding 77-joint indices
        positions_77[:, :, skel_slice, :] = positions_30
