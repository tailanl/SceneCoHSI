#!/bin/bash
# ============================================================
# Experiment 3: Rewrite Layer — SceneCo on ROOT MODEL only
# root: SA -> SceneCo -> FFN | body: SA -> FFN (original)
# Uses GPU 4 | batch=12 | ~1 day
# ============================================================
set -e
...
echo "=============================================="
echo " Experiment 3: Root-Only SceneCo (GPU 4)"
echo " root=SA->SceneCo->FFN | body=SA->FFN"
...
python -m kimodo_sceneco.exp.train_exp \
    ...
    --freeze_pretrained \
    --output_dir ./exp3_root_only_output \
    --batch_size 12 \
    --accum_steps 1 \
    --lr 1e-4 \
    --weight_decay 0.01 \
