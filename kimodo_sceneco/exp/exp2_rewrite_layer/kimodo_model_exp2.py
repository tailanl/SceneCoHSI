...
from .backbone_exp2 import migrate_pretrained_weights

log = logging.getLogger(__name__)


class KimodoSceneCoExp2(nn.Module):
    ...
    def _load_and_migrate_pretrained(self, denoiser, pretrained_denoiser):
        for block_name in ["root_model", "body_model"]:
            new_block = getattr(denoiser, block_name)
            old_block = getattr(pretrained_denoiser, block_name)
            ...
            shared_sd = {
                k: v for k, v in old_block.state_dict().items()
                if any(k.startswith(prefix) for prefix in shared_keys)
            }
            new_block.load_state_dict(shared_sd, strict=False)

            from .backbone_exp2 import SceneCoPostNormEncoder

            if isinstance(new_block.seqTransEncoder, SceneCoPostNormEncoder):
                migrate_pretrained_weights(new_block.seqTransEncoder, old_block.seqTransEncoder)
            else:
                new_block.seqTransEncoder.load_state_dict(old_block.seqTransEncoder.state_dict())
