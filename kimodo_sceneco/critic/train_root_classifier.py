...
def main():
    ...
    parser.add_argument("--output_dir", type=str, default="outputs/root_path_scene_classifier")
    parser.add_argument("--cache_dir", type=str,
                        default="LINGO/dataset/dataset/lingo_smplx_cache")
    ...
    if args.config and Path(args.config).exists():
        ...
        if "data" in cfg:
            d = cfg["data"]
            if "cache_dir" in d and not any("--cache_dir" in a for a in sys.argv):
                args.cache_dir = d["cache_dir"]
            if "max_frames" in d and not any("--max_frames" in a for a in sys.argv):
                args.max_frames = d["max_frames"]
        if "negative_sampling" in cfg:
            n = cfg["negative_sampling"]
            if "positive_ratio" in n and not any("--positive_ratio" in a for a in sys.argv):
                args.positive_ratio = n["positive_ratio"]
            if "modes" in n and not any("--negative_modes" in a for a in sys.argv):
