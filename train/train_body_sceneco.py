#!/usr/bin/env python
"""Train SceneCo body denoiser with GT root conditioning.

Key design:
  1. GT local_root replaces root model prediction (root model skipped entirely)
  2. Only body features contribute to loss (root loss zeroed)
  3. Root model is frozen; body pretrained weights frozen; SceneCo+TrajCo+VoxelViT trainable

Usage:
  CUDA_VISIBLE_DEVICES=0 python kimodo_scene_project/train/train_body_sceneco.py \
    kimodo_scene_project/configs/trajco_body_sceneco_gt_root.yaml
"""
...
def build_body_sceneco_model(config, device, ckpt_path=None):
    """Build KimodoSceneCo model for body-only SceneCo training.

    Patches the inner denoiser.forward to:
      - Skip root model entirely (frozen)
      - Compute GT local_root from gt_motion
      - Feed body model with GT local_root + scene/traj features
    """
    ...
    use_trajco_root = trajco_cfg.get("use_trajco_root", False)
    ...
    model = KimodoSceneCo(
        ...
        use_trajco=use_trajco_body or use_trajco_root,  # needed for traj_encoder creation
    ...
    def _patched_forward(_self, x, x_pad_mask, text_feat, text_feat_pad_mask, timesteps,
        ...
        # Compute GT local_root directly (skip frozen root model)
    ...
    # ----- Freeze: root_model completely, body pretrained only SceneCo+TrajCo+VoxelViT trainable -----
    for p in inner_denoiser.root_model.parameters():
        p.requires_grad = False

    # Use freeze_for_sceneco logic but also unfreeze trajco
    ...
    log.info(f"Body SceneCo model: trainable={trainable:,} / total={total:,} ({100*trainable/max(1,total):.1f}%)")
...
class BodySceneCoLoss(nn.Module):
...
def log_first_batch(model, batch, device):
    ...
    root_layers = getattr(model.denoiser.model.root_model, 'sceneco_layers', None)
    ...
    log.info(f"  sceneco_layers: root={len(root_layers) if root_layers else 0}, body={len(body_layers) if body_layers else 0}")
    log.info(f"  trajco_layers (body): {len(body_trajco) if body_trajco else 0}")
    log.info("=" * 60)
...
def prepare_batch(model, batch, device, training=True):
    ...
    Mirrors Trainer._prepare_batch from kimodo_sceneco.train.train.
    """
    ...
    # Text features: use zeros (we train scene-conditioned, not text-conditioned)
    B = motion.shape[0]
...
def main():
    ...
    num_epochs = train_cfg.get("num_epochs", 400)
    ...
    # ----- Build model -----
    log.info("Building body-only SceneCo model ...")
    ckpt_path = args.resume
    model = build_body_sceneco_model(cfg, device, ckpt_path)
    ...
    loss_fn = BodySceneCoLoss(model.diffusion, loss_mask)
    ...
    log.info(f"TrajCo: root={trajco_cfg.get('use_trajco_root',False)}, body={trajco_cfg.get('use_trajco_body',False)}")
