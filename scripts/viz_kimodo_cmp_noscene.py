"""Compare Kimodo two-stage (root=GT, text) vs GT — with post-processing to enforce initial pose match."""
...
def generate_with_root(model, motion_rep, sample, device, num_steps=50):
    T = sample["num_frames"]
    ...
    # Condition on GT root trajectory
    observed_motion = torch.zeros(1, T, D, device=device)
    motion_mask = torch.zeros(1, T, D, device=device)
    observed_motion[..., root_slice] = motion_rep.normalize(feat_t)[..., root_slice]
    ...
    # Use GT's first heading angle (not zero)
    ...
    # Post-processing: align root and force initial pose match
    # 1. Replace generated root with GT root
    delta = gt_root_np - gen_smooth
    gen_joints += delta[:, None, :]

    # 2. Force frame-0 body joints to match GT exactly
    gen_joints[0] = gt_joints[0].copy()

    return gen_joints, gt_root_np, gt_joints
...
def main():
    ...
    for si, sample in enumerate(samples):
        ...
        out_path = out_dir / f"cmp_{ci:05d}.mp4"

        gen_joints, gt_root, gt_joints = generate_with_root(
        ...
        angle_err = np.mean(np.abs(
            np.arccos(np.clip(np.sum(
                (gt_joints[2:] - gt_joints[1:-1]) * (gen_joints[2:] - gen_joints[1:-1]), axis=-1
            ) / (
                np.linalg.norm(gt_joints[2:] - gt_joints[1:-1], axis=-1) *
                np.linalg.norm(gen_joints[2:] - gen_joints[1:-1], axis=-1) + 1e-8
            ), -1, 1))
        )) * 180 / np.pi
