#!/usr/bin/env python
"""Batch evaluation + visualization for the 3 experiments.
...
Usage:
    CUDA_VISIBLE_DEVICES=0 python kimodo_scene_project/eval/batch_eval_3exp.py --gpu 0
"""
...
def build_model(exp_config, device):
    ...
    ckpt_path = find_latest_ckpt(Path(exp_config["ckpt_dir"]))
    ...
    for k, v in state_dict.items():
        if k.startswith("scene_encoder.") and k not in state_dict and not exp_config["dual_vit"]:
    ...
    print(f"    Step: {ckpt.get('step', '?')}, alphas:")
...
def main():
    ...
    output_root = PROJECT_ROOT / "kimodo_scene_project" / "outputs" / "eval_3exp"
    ...
    for exp_key, exp_cfg in EXPERIMENTS.items():
        ...
        exp_results = {"config": exp_cfg, "results": []}
    ...
    with open(summary_path, "w") as f:
        f.write("3-Experiment Evaluation Summary\n")
        f.write("=" * 60 + "\n\n")
        for exp_key, exp_cfg in EXPERIMENTS.items():
            f.write(f"{exp_cfg['label']} ({exp_key})\n")
            f.write(f"  checkpoint_dir: {exp_cfg['ckpt_dir']}\n\n")
